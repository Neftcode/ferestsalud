...
function posicionarMenu(val=false) {
    var alturaHeader = $(".jumbotron").outerHeight(true);
    var alturaMenu = $(".navbar").outerHeight(true);
    if ($(".contenido").scrollTop() >= alturaHeader) {
        ...
        if (val) {// es verdadero cuando se oprimee el botón del menú
            $("#progress_bar").animate({"position": "", "top": '', "width": "", "zIndex": ''}, 0, () => {
                setTimeout(() => {
                    $("#progress_bar").css({"position": "fixed", "top": $(".navbar").outerHeight(), "width": "100%", "zIndex": 1000});
                    heightContent();
                }, 350);
            });
...
function navMenuChange() {
    ...
    if (newElems.length) {
        var resp = newElems.some((elem, idx, arr) => {
            ...
            var top = elem[0].offsetTop-140;
            var bottom = (elem[0].offsetTop-140)+elem.outerHeight(true);
            if (idx<arr.length-1) {// Validar si la posición actual es menos a la últma posición del arreglo
                var topNext = elemNext[0].offsetTop-140;                
                // var bottomNext = (elemNext[0].offsetTop-140)+elemNext.outerHeight(true);
                if (posicionActual>=top&&posicionActual<topNext) {
                    $("#menu_navbar").find("a.active").removeClass("active text-white").addClass("text-white-transparent");
                    $("#menu_navbar").find("a").eq(idx).removeClass("text-white-transparent").addClass("active text-white");
                    return true;
                }
            } else {
        ...
        if (resp!==true) $("#menu_navbar").find("a.active").removeClass("active text-white").addClass("text-white-transparent");
    }
}
// Función para hacer scroll
function doScroll(elem) {
    $("body,.contenido").animate({ scrollTop: document.getElementById(elem).offsetTop-140 }, 1000);
}
